...
import type { Form, FormItem } from 'tyform/types';
  ...
  render() {
    const builder = tyform<Contact>({
      ...
      Notification: boolean<Contact>()
        .validate(
          (value, target) =>
            value && (!!target?.Phone.value || !!target?.Email.value),
          ['Phone', 'Email']
        )
        .withMessage(
    ...
    builder.subscribe((form) => {
      this.invalid = Object.values(form).reduce(
        (acc, cur) => acc || cur.invalid,
        false
      );
    });
    ...
    return html`
      <form>
        <label>
          Name:
          <input
            class="${classMap({ invalid: this.form.Name.invalid })}"
            type="text"
            .value=${this.form.Name.value}
            @input=${HandleInput(this.form.Name)}
          />
        ...
        <label>
          Phone:
          <input
            class="${classMap({ invalid: this.form.Phone.invalid })}"
            type="text"
            .value=${this.form.Phone.value}
            @input=${HandleInput(this.form.Phone)}
          />
        ...
        <label>
          Email:
          <input
            class="${classMap({ invalid: this.form.Email.invalid })}"
            type="text"
            .value=${this.form.Email.value}
            @input=${HandleInput(this.form.Email)}
          />
        ...
        <label>
          Notification:
          <input
            class="${classMap({ invalid: this.form.Notification.invalid })}"
            type="checkbox"
            ?checked=${this.form.Notification.value}
            @change=${HandleChecked(this.form.Notification)}
          />
