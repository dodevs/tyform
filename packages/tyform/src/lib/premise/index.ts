import type {
    ...
    FormOptions,
    FormOptionsChain,
    FormOptionsChainConstructor,
    ValidationMap,
    WithBindErrorOption,
    WithValidationMessageOption,
  } from '../../types';
  ...
  export const t = <T, O>(
    ...
    const Premise = function (this: FormOptionsChain<T, O>) {
      ...
      this.required = (message?: string) => {
        ...
        if (message) validationMap.message = message;
      ...
      this.validate = (fn, deps) => {
        ...
        const validationMap: ValidationMap<T, O> = {
          fn: (value, target) => !value || fn(value, target),
          deps,
        };
        options.validate.push(validationMap);
      ...
      this.bind = (selector) => {
        ...
        if (element.tagName != 'INPUT') element = element.querySelector('input');
  
        if (!element)
          throw new Error(
            `Element with selector ${selector} don't have a inner input`
          );
