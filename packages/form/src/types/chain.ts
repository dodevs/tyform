...
type ValidationMessageOption<T, O> = (message: string) => FormOptionsChain<T, O>;
...
type ValidateOption<T, O> = (fn: ValidationFn<T, O>, deps?: (keyof O)[])  => FormOptionsChain<T, O> & WithValidationMessageOption<T, O>;
type TransformOption<T, O> = (fn: (value: T) => T) => FormOptionsChain<T, O>;
type BindOption<T, O> = (selector: string) => FormOptionsChain<T, O> & WithBindErrorOption<T, O>;
...
export type FormOptionsChainConstructor<T, O> = {new (): FormOptionsChain<T, O>}